    ...
    <div class="info-panel">
      ...
      <div class="signal-kpis">
        ...
        <div class="kpi-item">
          ...
          <div class="kpi-value" :class="activeSignalState === 'off' ? 'danger' : 'ok'">{{ activeSignalState === 'off' ? '异常灭灯' : '正常' }}</div>
        </div>
        <div class="kpi-item">
    ...
    <div class="stats-panel">
      ...
      <div class="panel-section">
        ...
        <div class="shortcut-grid">
          ...
          <button class="shortcut-card" @click="openDeviceModal('signal', selectedSignalId)">
            <span class="shortcut-title">{{ heroDeviceName }}</span>
      ...
      <div class="panel-section">
        ...
        <div class="key-grid">
          ...
          <div class="key-chip"><b>L</b><span>1495 轨道电路故障掉压</span></div>
        </div>
      </div>
...
const selectedSignalId = ref('1495')
const selectedSignal = computed(() => signalCatalog.find((item) => item.id === selectedSignalId.value) || signalCatalog[2])
...
const signalHardwareReady = ref(false)
const signalFaultActive = ref(false)
...
const activeSignalState = computed(() => {
  const target = signals.find((item) => item.id === selectedSignalId.value)
  return target?.state || 'red'
})
...
const alertFeed = computed(() => {
  ...
  if (trackFaultActive.value) {
    items.push({
      ...
      level: 'danger',
      title: '1495调谐单元异常',
      description: 'FS 调谐单元内部电容受潮，谐振频率偏离设计值，轨道电压降为 0V。'
    })
  }
  if (!items.length) {
    items.push({
...
const deviceStatusRows = computed(() => ([
  ...
  { label: '散热系统', value: signalFaultActive.value ? '异常' : '正常', className: signalFaultActive.value ? 'warning' : 'online' },
...
const modalCurrentMetric = computed(() => {
  switch (deviceModal.value.type) {
    case 'power':
      return telemetryConnected.value ? 380 : 0
    case 'turnout':
      return signalHardwareReady.value ? 110 : 0
    case 'track':
      return trackFaultActive.value ? 0 : (signalHardwareReady.value ? 72 : 0)
    case 'signal':
    default:
      return signalFaultActive.value ? 0 : (signalHardwareReady.value ? signalParams.value.voltage : 0)
  }
})
...
const modalAlertText = computed(() => {
  if (deviceModal.value.type === 'signal' && signalFaultActive.value) {
    return '1495信号机异常灭灯，左侧已同步推送告警。'
  }
  if (deviceModal.value.type === 'track' && trackFaultActive.value) {
    return '1495调谐单元故障持续中，请按要求携带备件前往 K149+525 处处理。'
  }
  if (deviceModal.value.type === 'power' && !telemetryConnected.value) {
...
const modalChartColor = computed(() => {
  if ((deviceModal.value.type === 'signal' && signalFaultActive.value) || (deviceModal.value.type === 'track' && trackFaultActive.value)) {
    return '#ff5c5c'
  }
  return '#00d4ff'
})
...
const modalSeriesData = computed(() => {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const wave = Math.sin((now / 1000) * 0.9 + index * 0.55)
    const ripple = Math.cos((now / 1400) + index * 0.35)
    let value = 0

    if (deviceModal.value.type === 'power') {
      value = telemetryConnected.value ? 370 + wave * 8 + ripple * 3 : 0
    } else if (deviceModal.value.type === 'turnout') {
      value = signalHardwareReady.value ? 108 + wave * 5 + ripple * 2 : 0
    } else if (deviceModal.value.type === 'track') {
      value = trackFaultActive.value ? 0 : (signalHardwareReady.value ? 71 + wave * 2.8 + ripple : 0)
    } else {
      value = signalFaultActive.value ? 0 : (signalHardwareReady.value ? selectedSignal.value.voltageBase + wave * 4 + ripple * 1.5 : 0)
    }

    points.push({
      time: new Date(now - (total - 1 - index) * 60 * 1000).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }),
...
const modalPreviewStats = computed(() => {
  if (deviceModal.value.type === 'track') {
    return [
      { label: '轨道电压', value: `${trackFaultActive.value ? 0 : (signalHardwareReady.value ? 72 : 0)}V` },
      { label: '调谐电容', value: `${observedTrackCapacitance.value.toFixed(1)}uF` },
  ...
  return [
    ...
    { label: '接入状态', value: telemetryConnected.value ? '已接入' : '未接入' },
    { label: '系统时间', value: new Date(uiNowMs.value).toLocaleTimeString('zh-CN') }
  ]
})
...
const syncSignalVisual = (signalId, state) => {
  const target = signals.find((item) => item.id === signalId)
  if (!target) return
  target.state = state
  const color = getColorByState(state)
  if (target.light) {
    target.light.color.setHex(color)
    target.light.intensity = state === 'off' ? 0 : 3
  }
  if (target.mesh) {
    target.mesh.traverse((child) => {
      ...
      if (child.material.emissive) {
        ...
        if ('emissiveIntensity' in child.material) {
          child.material.emissiveIntensity = state === 'off' ? 0 : 0.7
        }
      }
      if (child.material.color) {
        child.material.color.setHex(state === 'off' ? 0x555555 : color)
      }
    })
  }
}
...
const restoreNormalState = () => {
  signalFaultActive.value = false
  trackFaultActive.value = false
  observedTrackCapacitance.value = 18.6
  SIGNAL_IDS.forEach((id) => syncSignalVisual(id, id === '1495' ? 'red' : 'green'))
  updateSignalUI()
  closeDeviceModal()
}
...
const onGlobalKeydown = (event) => {
  ...
  if (key === 'm' || key === 'M') {
    telemetryConnected.value = true
    signalHardwareReady.value = true
    signalFaultActive.value = false
    observedTrackCapacitance.value = trackFaultActive.value ? 8.3 : 18.6
    syncSignalVisual('1495', 'red')
    openDeviceModal('signal', '1495')
    return
  }
  if (key === 'p' || key === 'P') {
    telemetryConnected.value = true
    signalHardwareReady.value = true
    signalFaultActive.value = true
    selectedSignalId.value = '1495'
    syncSignalVisual('1495', 'off')
    openDeviceModal('signal', '1495')
    speakAlarm('检测到1495信号机内部温度异常，漏电严重，建议携带工具箱、密封圈及LED信号机点灯单元前往处理。')
    return
  }
  if (key === 'l' || key === 'L') {
    telemetryConnected.value = true
    ...
    speakAlarm('监测到1495调谐单元内部电容受潮容值改变，现谐振频率偏离设计值，请携带调谐单元备件及工具箱前往K149加525处进行更换作业。')
    return
  }
  if (key === 'i' || key === 'I') {
...
const createSignalLights = () => {
  ...
  const signalStates = ['green', 'green', 'red', 'green']
...
const updateSignalUI = () => {
  ...
  signalList.innerHTML = signals.map(signal => `
    <div class="signal-item ${selectedSignalId.value === signal.id ? 'signal-item-active' : ''}" data-signal-id="${signal.id}">
      ...
      <div class="signal-info">
        ...
        <div class="signal-meta-row">
          <span>点击查看监测</span>
          <span>${signal.id === '1495' && signalFaultActive.value ? '异常处理中' : '在线'}</span>
        </div>
      </div>
    </div>
  `).join('')

  signalList.onclick = (event) => {
    const target = event.target instanceof Element ? event.target.closest('[data-signal-id]') : null
    const signalId = target?.getAttribute('data-signal-id')
    if (!signalId) return
    handleSignalAction(signalId)
  }
}
...
const toggleSignals = () => {
  ...
  signals.forEach(signal => {
    if (signal.id === '1495' && signalFaultActive.value) {
      syncSignalVisual(signal.id, 'off')
      return
    }
    ...
    signal.light.intensity = signal.state === 'off' ? 0 : 3
...
const updateSignalParams = async () => {
  ...
  signalParams.value.voltage = signalFaultActive.value
    ? 0
    : (signalHardwareReady.value ? Math.round((base.voltageBase - 2 + Math.random() * 4) * 10) / 10 : 0)
  signalParams.value.current = signalFaultActive.value
    ? 0
  ...
  signalTwin.value.interlocking = signalFaultActive.value || trackFaultActive.value ? '预警' : (health < 70 ? '预警' : '正常')
  signalTwin.value.workMode = signalFaultActive.value || trackFaultActive.value ? '故障演练' : (latency > 320 ? '降级' : '自动')
